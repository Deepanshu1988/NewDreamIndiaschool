  ...
  const insertImage = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4400/api/auth/add-gallery-image",
        formData,
        {
          headers: {
  ...
  const handleImageDelete = async (id) => {
    try {
      const res = await axios.delete(
        `http://localhost:4400/api/auth/deleteImages/${id}`
      );
    ...
    const handleUpdateImage = async (e) => {
      ...
      try {
        const res = await axios.put(
          `http://localhost:4400/api/auth/update-image/${selectedItem.item_id}`,
